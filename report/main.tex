...
After having fetched a block of valid measurement values, which has zero in stale bit (introduced in \ref{sec:sensor_hyt281},
the status bits will be removed.
Then a CRC-8 checksum will be computed and attached after the data block.
After that, the whole data will be encoded as ZACware signals and sent out.
Figure \ref{fig:hyt_board_transmission} show the new transmission structure over the ZACware protocol.
\begin{figure}[Hh!]
	\centering
	\definecolor{lightgray}{gray}{0.8}
	\begin{bytefield}[endianness=big, bitwidth=2.1em]{8}
    ...
    \bitbox{8}{CRC-8}
	\end{bytefield}
  \caption{Transmission structure over the ZACware protocol, which is sent out from hymidity board. The two bits, marked with gray color, before the temperature value are not care.}
