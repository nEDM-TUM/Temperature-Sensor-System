...
\end{figure}
\subsection{HYT271}
\label{sec:sensor_hyt281}
For humidity measurement, we use the HYT271 sensor. At a price of about \EUR{20} per device it is significantly more expensive than the temperature sensors, however it delivers precise and temperature compensated humidity measurements. 
...
\begin{figure}[Hh!]
	...
	\begin{bytefield}[endianness=big, bitwidth=2.1em]{8}
		\bitheader{0-7}\\
    \bitbox{2}{Status} & \bitbox[lrt]{6}{}\\
    \wordbox[lrb]{1}{Humidity}\\
    \bitbox{2}{\color{lightgray}\rule{\width}{\height}} & \bitbox[lrt]{6}{}\\
    \wordbox[lrb]{1}{Temperature}\\
	\end{bytefield}
  \caption{HYT271 transmission structure. The humidity value and temperature value each consist of 14 bits. The two bits, marked with gray color, before the temperature value are undefined \cite{hyt2}.}
...
\subsubsection{Transmission structure}
After having fetched a block of valid measurement values, whose stale bit is zero (see section \ref{sec:sensor_hyt281}),
the status bits will be removed.
Then a CRC-8 checksum will be computed and attached after the data block.
After that, the whole data will be encoded to ZACwire signals and sent out.
Figure \ref{fig:hyt_board_transmission} shows the new transmission structure, of the ZACwire transmission.
\begin{figure}[Hh!]
	\centering
	\definecolor{lightgray}{gray}{0.8}
	\begin{bytefield}[endianness=big, bitwidth=2.1em]{8}
		\bitheader{0-7}\\
    \bitbox[b]{7}{} & \bitbox{1}{ST}\\
    \bitbox{2}{0} & \bitbox[lrt]{6}{}\\
    \wordbox[lrb]{1}{Humidity}\\
    \bitbox{2}{\color{lightgray}\rule{\width}{\height}} & \bitbox[lrt]{6}{}\\
    \wordbox[lrb]{1}{Temperature}\\
    \bitbox{8}{CRC-8}
	\end{bytefield}
  \caption{ZACwire Transmission structure of data, which is sent out from the humidity board. The two bits, marked with gray color, before the temperature value are undefined. ST represents the start bit.}
	\label{fig:hyt_board_transmission}
\end{figure}\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
After a overflow occurs, we check, if we received a whole transmission.
This is done, by filling the receive buffer with all ones prior to receiving. Then we use the fact, that
the TSIC sensor, as well as the humidity board start the transmission with a 0 bit (see figure \ref{fig:tsic_transmission} and \ref{fig:hyt_board_transmission}.
Hence, if we see a
0 on the expected position of a humidity readout or a TSIC readout after a timeout, we might have
enough data and stop the sampling, as the transmission is most likely complete.\\
Sampling is stopped by disabling the
interrupts. However, if we did not receive enough data, we reset the receive buffer and wait for the
next transmission to begin.\\
As the transmission of a humidity board is longer than that of a TSIC sensor, we later will be able to determine the
type of the sensor, by looking at the leftmost bit in the receive buffer, where the first 0 bit of the humidity
board should be. If this bit is 0, we know, that the sample is from a humidity board, else it is from a TSIC
sensor.
...
\item To allow EEPROM storage to stay intact after reprogramming, set the
  corresponding fuse bit.
\item As Arduino fuses are not possible to be programmed via the default Arduino
  programmer (using the Arduino boot loader), we removed the boot loader and use
...
\item When debugging and programming devices of the sensor network, one has to
  ...
  as connected computers. By default the USBasp programmer provides the system
  with 5V power.
...
\begin{figure}[Hh!]
	...
	\caption{Temperature board schematic. The Impedance adjuster circuit, R4 and C4 is not required to be populated, as we only transmit data.}
	\label{fig:schem_temp}
\end{figure}
\begin{figure}[Hh!]
	\centering
	\includegraphics[width=0.9\textwidth]{img/schem_humidity_board.pdf}
	\caption{Humidity board schematic.The Impedance adjuster circuit, R6 and C5 is not required to be populated, as we only transmit data.}
	\label{fig:schem_hum}
\end{figure}
\begin{figure}
	\centering
...
%###############################################################
\chapter{Installation images and schematics}
\begin{figure}[Hh!]
	\centering
